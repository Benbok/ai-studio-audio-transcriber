...
Приложение также может быть запущено как настольное приложение с помощью Electron.

### Разработка (Development Mode)

Запустите приложение в режиме разработки с горячей перезагрузкой:

```bash
npm run electron:dev
```

Это запустит Vite dev server и Electron одновременно. Приложение откроется в окне Electron с включенными DevTools.

### Сборка для продакшена (Production Build)

Соберите исполняемый файл для Windows:
...
### Конфигурация переменных окружения для Electron

В режиме разработки Electron читает `.env.local` из корня проекта (как и обычный веб-режим).

В продакшене (собранное приложение) Electron ищет `.env.local` в той же директории, где находится исполняемый файл (`.exe`). Просто поместите файл `.env.local` рядом с `.exe` файлом.

**Важно:** В Electron приоритет переменных окружения следующий:
1. `window.electronEnv` (из `.env.local` в директории приложения)
2. `import.meta.env` (Vite env vars)
3. `process.env` (системные переменные окружения)

Это означает, что `.env.local` рядом с исполняемым файлом имеет наивысший приоритет в продакшене.
